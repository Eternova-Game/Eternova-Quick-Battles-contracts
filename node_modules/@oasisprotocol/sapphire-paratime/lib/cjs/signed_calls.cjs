...
_SignedCallDataPack_instances = new WeakSet(), _SignedCallDataPack_encode = function _SignedCallDataPack_encode(data) {
    return ethers6_1.ethers.hexlify(cbor.encode({
...
function parseBytesLike(data) {
    ...
    return ethers6_1.ethers.getBytesCopy(data);
}
function stringifyBytesLike(data) {
    if (Array.isArray(data))
        return ethers6_1.ethers.hexlify(new Uint8Array(data));
    return ethers6_1.ethers.hexlify(data);
}
function makeLeash(signer, overrides) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        return {
            ...
            block_hash: ethers6_1.ethers.getBytesCopy(block.hash),
...
function signCall(call, signer, overrides) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const hash = ethers6_1.ethers.TypedDataEncoder.hash(upgradedDomain, types, upgradedCall);
        let signature = _cache.get(address, hash);
        // if (signature !== undefined) return signature;
        if ('_signTypedData' in signer) {
            signature = ethers6_1.ethers.getBytes(yield signer._signTypedData(domain, types, call));
        }
        else {
            signature = ethers6_1.ethers.getBytes(yield signer.signTypedData(upgradedDomain, types, upgradedCall));
        }
...
function upgradeDomain(domain) {
    return Object.assign(Object.assign({}, domain), { salt: domain.salt ? parseBytesLike(domain.salt) : undefined, chainId: domain.chainId
            ? bignumber_1.BigNumber.from(domain.chainId).toHexString()
            : undefined });
}
function upgradeCall(call) {
    const big2int = (b) => {
        ...
        if (typeof b === 'string')
            return b;
        if (b instanceof bignumber_1.BigNumber)
            return b.toHexString();
        return ethers6_1.ethers.toQuantity(b);
    };
    return Object.assign(Object.assign({}, call), { gasPrice: big2int(call.gasPrice), value: big2int(call.value), data: call.data ? ethers6_1.ethers.getBytes(call.data) : undefined });
}
